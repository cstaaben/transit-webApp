...
function makeRoutes(results,j){
	...
	for(var i = 0; i < results.routes[j].legs[0].steps.length; i++){
		if(results.routes[j].legs[0].steps[i].travel_mode === "TRANSIT"){
			stepString += '<i class="ui big bus icon pRouteBusIcon"></i>'; //<p> ' + results.routes[j].legs[0].steps[i].transit.line.short_name + ' </p> //TODO: integrate into display; make it look nice
